...
Our program is implemented using the [Julia programming language](julialang.org/).
It can be easily scaled to multiple computing nodes with Julia's built-in support for [parallel computing](https://docs.julialang.org/en/latest/manual/parallel-computing).
...
Once installed, launch Julia from the terminal by executing `julia` and install required packages via the following commands (within Julia).
```julia
julia> Pkg.add("JLD")
julia> Pkg.add("HDF5")
...
We used three datasets in out paper [1], a synthetic one, one from ImageNet, and one from [NYT Corpus](https://catalog.ldc.upenn.edu/ldc2008t19).  
...
```
 `101` is the random seed that generates the data used in the paper. You can change other seeds.
 ...
 Download them to your local disks and modify paths in [`ExperimentSetting.jl`](ExperimentSetting.jl) accordingly.

You can also use your own dataset.
The program takes a matrix with all feature vectors and a vector of all labels.
Remember add an entry in [`ExperimentSetting.jl`](ExperimentSetting.jl).
...
Available dataset can be found in [`ExperimentSetting.jl`](ExperimentSetting.jl).

The program with output the summary of the result every iteration like:
```
Iter = 50    ** This is the 50-th iteration **
 runtime = 1.602912625    ** This iteration takes these seconds to run **
 # of Clusters = 60, Size = [1, 3, 8, 12, 87, 177, 310, 348, 367, 452, 460, 542, 595, 793, 917, 967, 983, 997, 999, 1003, 1004, 1005, 1008, 1014, 1080, 1774, 1912, 1912, 1913, 1983, 1984, 1987, 1995, 2000, 2004, 2011, 2020, 2022, 2029, 2069, 2090, 3358, 4235, 4710, 4968, 4988, 4990, 4995, 4998, 5000, 5000, 5000, 5002, 5002, 5012, 5058, 5059, 5080, 5223, 6485]    ** There are totally 60 clusters proposed, their sizes (number of samples contained) are [1, 3 ... 6485] respectively, sorted ascendingly **
 ...
 loglikelihood = -929192.2782973386    ** the Likelihood of the model **
 vi = 0.7586535041975777    ** Variational Information criteria (see paper [1] for detail) **
 count = 60.0    ** Total number of proposed components, same as above **
```

This is the desired result.

## Test the CGS baseline multiple times

You may need to run the baseline for multiple times and average the results.
To do so, please follow below procedures.  

0. Mare sure the path is correctly set in [`ExperimentSetting.jl`](ExperimentSetting.jl).
1. Launch Julia with multiple workers (5 workers here). Each worker will reside on a CPU core.
```bash
$ julia -p 5
```
2. Include the code
...
But unlike the serial Gibbs sampler, the proposed methods are implemented in a parallel manner.
...
```
The above code will run pooled consolidation using 20 workers and for 50 iterations. The third argument indicates the number of Metropolis-Hasting steps performed in the fusion center. See section 4.2.2 in the paper [1] for detail.
- Asynchronous algorithm
```julia
julia> include("evalAsync.jl"); evalAsyncDistribute(20, 50, 100, :s)
```
3. Evaluate the algorithm for multiple times (3 in below examples)
- Progressive consolidation
```julia
...
We also implemented other baselines, including AVparallel [2] (a improved version), SubC [3], SliceMR [4] and a modification of our proposed method using the Hungarian merging policy proposed in [5], in Julia.
...
These baselines are also implemented in a parallel manner. So in order to evaluate them, launch Julia with multiple workers. Detailed instructions are list as follows.
...
All of them can be evaluated under multi-node setting.
...
### Install Julia
Install the Julia programming language on all servers and set the  `$PATH` environment variable correctly.
Make sure that, on each server, by typing `julia` from the console, Julia is launched correctly.
...
After this step, you should be able to login between any two servers in both direction.
Note that for the first time to login, the server will prompt for key fingerprint verification.
You need to manually accept it once before running the Julia client, as it will block the Julia client connecting between servers.

Just make sure from either server, after typing `ssh $EITHER_IP_ADDRESS`, the console of the destination server appears without any interruption.
...
```julia
addprocs([("10.1.72.21",15),("10.1.72.22",15)])
```

This command will start 15 workers on both servers.
There will be 30 workers in total.

If success, the command will return a list of worker ids like :
```julia
...
```
We modified the second argument from 20 to 30.
This program will evaluate the progressive consolidation using 30 workers.

The initialization step could be rather slow, due to code compilation and data distribution. Just be patient. 
